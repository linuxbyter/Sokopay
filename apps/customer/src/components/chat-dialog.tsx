...
import { Send, ArrowLeft, CheckCircle, Package, Truck, MessageSquare, Star, RotateCcw, Flag } from 'lucide-react';
...
export default function ChatDialog({ chat, onBack, onChatUpdate, isVendor }: ChatDialogProps) {
  ...
  const renderCategoryQuickReplies = () => {
    ...
    return (
      <div className="px-3 pb-2 pt-1 bg-background border-t border-border">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-text-tertiary font-medium">Quick messages</span>
          <button onClick={() => setQuickRepliesDismissed(true)} className="text-xs text-text-tertiary hover:text-text-secondary px-1">✕</button>
        </div>
        <div className="flex gap-2 flex-wrap">
          {replies.map((reply) => (
            <button
              ...
              className="px-3 py-1.5 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-medium rounded-full border border-primary/20 transition-colors whitespace-nowrap"
            >
              {reply}
            </button>
          ))}
  ...
  const renderQuickActions = () => {
    if (activeChat.is_finalized) {
      return (
        <div className="flex gap-2 flex-wrap p-3 bg-surface border-t border-border">
          <button
            onClick={handleBuyAgain}
            className="flex items-center gap-2 px-4 py-3 bg-primary/10 text-primary rounded-lg text-sm font-medium hover:bg-primary/20 transition-colors min-h-[48px]"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-3 bg-copper-400/10 text-copper-400 rounded-lg text-sm font-medium hover:bg-copper-400/20 transition-colors min-h-[48px]"
          >
        ...
        return (
          <div className="flex gap-2 flex-wrap p-3 bg-surface border-t border-border">
            <button
              ...
              className="flex items-center gap-2 px-4 py-3 bg-success/10 text-success rounded-lg text-sm font-medium hover:bg-success/20 transition-colors min-h-[48px]"
            >
          ...
          <div key={step.label} className="flex items-center">
            <div className="flex flex-col items-center">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${
                step.done ? 'bg-primary text-primary-foreground' : 'bg-surface-hover text-text-tertiary'
              }`}>
            ...
            {index < steps.length - 1 && (
              <div className={`w-8 h-0.5 mx-1 ${step.done ? 'bg-primary' : 'bg-border'}`} />
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border bg-surface">
        <button
          onClick={onBack}
          className="p-3 hover:bg-surface-hover rounded-lg transition-colors min-h-[48px] min-w-[48px] flex items-center justify-center"
        >
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-text-secondary">
            {activeChat.vendor_category}
          </p>
        </div>
        <div className="flex items-center gap-1">
          <div className={`px-2 py-1 rounded-full text-xs font-medium ${
            activeChat.status === 'completed' ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'
          }`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-text-secondary">
            <MessageSquare className="w-12 h-12 mb-3 text-text-tertiary" />
            <p>No messages yet</p>
          ...
          messages.map((message) => (
            ...
            >
              {message.message_type === 'system' ? (
                <div className="px-3 py-1 bg-surface-hover rounded-full text-xs text-text-secondary border border-border">
                  {message.content}
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  <p className={`text-xs mt-1 ${
                    message.sender_id === user?.id ? 'text-primary-foreground/70' : 'text-text-tertiary'
                  }`}>
        ...
        <div className="p-4 border-t border-border bg-surface">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 bg-background border border-border rounded-full text-sm text-text-primary placeholder:text-text-tertiary focus:border-primary focus:outline-none min-h-[48px]"
              autoComplete="off"
            />
            <button
              onClick={() => sendMessage(newMessage)}
              disabled={!newMessage.trim() || sending}
              className="p-3 bg-primary text-primary-foreground rounded-full hover:bg-primary-hover transition-colors disabled:opacity-50 min-h-[48px] min-w-[48px] flex items-center justify-center"
            >
